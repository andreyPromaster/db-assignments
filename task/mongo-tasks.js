    ...
    const result = await db.collection("order-details").aggregate([
        {
            ...
            $project: {
                ...
                "Total Order Discount, %": {
                    $round: [{
                        $multiply: [{
                            $divide: ["$TotalDiscount", "$TotalOrder"]
                        }, 100]
                    }, 3]
                }
            }
    ...
    const result = await db.collection('orders').aggregate([
        ...
        },
        {
            $project : {
                ...
                "% of all orders" : {$round : [{ $multiply: [{$divide : ["$Total number of Orders", list[0].OrdersCount]}, 100]}, 3]}
            }
        },
    ...
    const result = await db.collection('products').aggregate([
        {
            ...
            $project: {
                "TotalOfCurrentProducts": {$add: prod[0].TotalOfCurrentProducts },
    ...
    const result = await db.collection('employees').aggregate([
        {
            ...
            $group: {
                ...
                "Amount, $": {
                    $sum: {
                        $multiply: ["$detail.UnitPrice", "$detail.Quantity"]
                    }
                }
            }
        }, {
            $project: {
                ...
                "Employee Full Name": "$_id.EmployeeFullName",
                "Amount, $": 1
            }
        }, {
            $sort: {
